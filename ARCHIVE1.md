...
Open your browser and visit the admin page at `http://localhost:1337/admin`. Login and view your dashboard.

The details to login to the admin are `chef@strapi.io` for the email and `Gourmet1234` for the password.
...
Open the **Content-Type Builder** to create a `Note` collection. This is for storing notes.
...
Click `+` Note collection with title(short text), content(long text) fields
